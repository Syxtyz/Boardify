...
export default function BoardView() {
  ...
  const DroppableList = ({
    ...
    return (
      <div
        ref={setNodeRef}
        className={`w-64 flex flex-col p-2 rounded bg-gray-100 dark:bg-zinc-900 ${
          isOver ? "bg-gray-300 dark:bg-zinc-700" : ""
        }`}
      >
        {children}
      </div>
    ...
    <>
      ...
      <ScrollArea className="h-[calc(100vh-6rem)]">
        <DndContext
          ...
          //   // Check if active is a list (only lists are directly in the lists array)
          //   const activeId = Number(active.id);
                  ...
                  <ScrollArea className="max-h-[73vh] overflow-y-auto flex flex-col">
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <CreateCard style="mx-1" onClick={() => createNewCard(list.id)} />
                      </div>
                    </SortableContext>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-5xl flex flex-col gap-2">
          {modalOpen && (
            <>
              <CardList
              ...
              />
              <div className="border-b-2 border-zinc-300 dark:border-zinc-700"></div>
              <div className="">
                {creatingCardState ? (
                  <CardForm
                    selectedList={selectedList}
...
//       const sourceCardIdx = lists[sourceListIdx].cards.findIndex(c => c.id === activeId)
//       let targetListIdx: number
//       let targetCardIdx: number
//       if (typeof overId === "string" && overId.startsWith("list-")) {
//         targetListIdx = lists.findIndex(l => `list-${l.id}` === overId)
...
//       } else {
//         const overIdNum = Number(overId)
//         targetListIdx = lists.findIndex(l => l.cards.some(c => c.id === overIdNum))
//         targetCardIdx = lists[targetListIdx].cards.findIndex(c => c.id === overIdNum)
//       }
//       if (sourceListIdx === targetListIdx && sourceCardIdx === targetCardIdx) {
//         setActiveCardId(null)
//         return
...
//   const SortableList = memo(({ list, children }: { list: List; children: React.ReactNode }) => {
...
//             const { active } = evt
//             if (!active) return
//             if (lists.some((l) => l.id === Number(active.id))) handleListDragEnd(evt)
//             else handleCardDragEnd(evt)
//           }}
//         >
...
//                       <div className="flex flex-col">
//                         {list.cards.map(card => <CardItem key={card.id} card={card} listId={list.id} />)}
...
//             {activeCardId != null ? (
//               (() => {
//                 const card = lists.flatMap(l => l.cards).find(c => c.id === activeCardId)
//                 if (!card) return null
...
//           {modalOpen && (
//             <>
//               <CardList selectedList={selectedList} setCreatingCard={setCreatingCardState} setIsEditing={setIsEditing} />
//               <div className="w-[45%] pl-4">
//                 {creatingCardState ? (
//                   <CardForm selectedList={selectedList} onCancel={() => setCreatingCardState(false)} />
//                 ) : selectedCard ? (
//                   isEditing ? (
//                     <CardForm selectedList={selectedList} isEditing onCancel={() => setIsEditing(false)} />
//                   ) : (
//                     <CardDetails onEdit={() => setIsEditing(true)} />
//                   )
//                 ) : (
//                   <p className="text-sm text-muted-foreground mt-4">Select a card to view or create a new one.</p>
//                 )}
//               </div>
//             </>
//           )}
